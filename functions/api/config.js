...
const cacheKeyFor = (request) =>
	new Request(new URL("/api/config", request.url).toString(), {
		method: "GET",
	});
...
export async function onRequestGet(context) {
	...
	let stored = null;
	try {
		stored = await getStore(env).getRaw(KEY);
	} catch (e) {
		console.error("config GET: KV read failed", e);
		stored = null;
	}
	...
	if (!stored) {
		...
		try {
			...
			const patch = {};
			const lists =
				cfg.squads && typeof cfg.squads === "object"
					? Object.values(cfg.squads)
					: [];
			const hasAnyClub = lists.some(
				(l) => Array.isArray(l) && l.some((p) => p && p.club),
			);
			if (!lists.length || !hasAnyClub) patch.squads = DEFAULT_CONFIG.squads;
			// schedule は常にコードの defaults.js を正本とする。
			// 古いKVに残るスロット表記スケジュール（R32が "1E"/"2B"/"3(A/B/C/D/F)" 等＝画面で「グループX 1位」表示）を無視し、
			// グループ確定後の実カード（defaults.js の R32 は実チームコード）を全利用者へ配信する。
			patch.schedule = DEFAULT_CONFIG.schedule;
			if (Object.keys(patch).length)
				body = JSON.stringify({ ...cfg, ...patch });
		} catch (e) {
			console.error("config GET: stored JSON parse failed", e);
		}
	}
	...
	if (siteKey) {
		try {
			body = JSON.stringify({ ...JSON.parse(body), turnstileSiteKey: siteKey });
		} catch (e) {
			console.error("config GET: site key inject failed", e);
		}
	}
...
export async function onRequestPut(context) {
	...
	try {
		...
		console.error("config PUT: invalid json", e);
