...
test("aliases 未指定なら空配列", () => {
	const { aliases, ...noAliases } = DEFAULT_CONFIG; // aliases を明示的に外す
	const r = validateConfig(noAliases);
	assert.equal(r.ok, true);
