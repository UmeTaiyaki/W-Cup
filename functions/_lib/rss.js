...
// データ系ハブ/速報枠を弾く。日程・放送/配信予定・順位/ランキングはアプリ本体が持つので不要。
const DATA_TITLE_RE =
	/テレビ放送|地上波|ネット配信|配信予定|放送予定|放送・配信|中継|視聴方法|視聴ガイド|キックオフ時間|試合日程|出場国一覧|対戦カード|組み合わせ抽選|ランキング|順位表/;

// W杯2026以外(Jリーグ等)・スタブ・広報/販促・データ系ハブを除外する。
export function filterWorldCup(items) {
	return items.filter((it) => {
		const cats = (it.categories || []).join(" ");
		if (STUB_RE.test(it.title)) return false;
		if (DATA_TITLE_RE.test(it.title)) return false; // 日程/放送/ランキング等(アプリ本体が持つ)
