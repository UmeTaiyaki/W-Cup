...
const detail = {
  id: 1, league_id: 732, season_id: 18017,
  starting_at: '2022-12-06 15:00:00', starting_at_timestamp: 1670338400, state_id: 8,
  result_info: 'X won',
  participants: [
    { id: 10, name: 'A', short_code: 'AAA', image_path: 'a.png', meta: { location: 'home' } },
    { id: 20, name: 'B', short_code: 'BBB', image_path: 'b.png', meta: { location: 'away' } },
  ],
  scores: [
    { type_id: 1525, participant_id: 10, score: { goals: 1, participant: 'home' }, description: 'CURRENT' },
    { type_id: 1525, participant_id: 20, score: { goals: 2, participant: 'away' }, description: 'CURRENT' },
  ],
  events: [{ id: 100, participant_id: 10, type_id: 14, player_name: 'P', minute: 70, sort_order: 1 }],
  statistics: [{ id: 1, type_id: 45, participant_id: 10, data: { value: 60 }, location: 'home' }],
};

test('fixtureDetailStatements: teams/fixture/event/stat の文を生成', () => {
  const specs = fixtureDetailStatements(detail, 1000);
  // 2 teams + 1 fixture + 1 event + 1 stat = 5
...
test('seasonFixturesStatements: チーム重複排除＋全fixture', () => {
  ...
  const fixtures = [
    {
      id: 1, league_id: 732, season_id: 26618, starting_at: 'x', starting_at_timestamp: 1, state_id: 1,
      participants: [
        { id: 10, name: 'A', short_code: 'AAA', image_path: 'a.png', meta: { location: 'home' } },
        { id: 20, name: 'B', short_code: 'BBB', image_path: 'b.png', meta: { location: 'away' } },
      ],
    ...
    {
      id: 2, league_id: 732, season_id: 26618, starting_at: 'y', starting_at_timestamp: 2, state_id: 1,
      participants: [
        { id: 10, name: 'A', short_code: 'AAA', image_path: 'a.png', meta: { location: 'home' } },
        { id: 30, name: 'C', short_code: 'CCC', image_path: 'c.png', meta: { location: 'away' } },
      ],
    },
  ];
...
test('runBatch: db.prepare→bind→batch に specs を流す', async () => {
  ...
  const db = {
    prepare(sql) {
      ...
      return { bind: (...a) => { calls.bound.push(a); return { sql, a }; } };
    },
    batch: async (stmts) => { calls.batched = stmts.length; return stmts.map(() => ({ success: true })); },
  };
  const specs = [{ sql: 'INSERT 1', args: [1] }, { sql: 'INSERT 2', args: [2, 3] }];
...
test('runBatch: 空 specs は何もせず空配列', async () => {
  let touched = false;
  const db = { prepare: () => { touched = true; }, batch: async () => { touched = true; } };
