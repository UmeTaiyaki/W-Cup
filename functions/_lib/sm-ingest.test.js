...
const fixtureDetail = {
  ...
  participants: [
    { id: 18551, name: 'Morocco', short_code: 'MAR', image_path: 'https://cdn/18551.png', meta: { location: 'home' } },
    { id: 18710, name: 'Spain', short_code: 'ESP', image_path: 'https://cdn/18710.png', meta: { location: 'away' } },
  ],
  scores: [
    { type_id: 1, participant_id: 18551, score: { goals: 0, participant: 'home' }, description: '1ST_HALF' },
    { type_id: 1525, participant_id: 18551, score: { goals: 1, participant: 'home' }, description: 'CURRENT' },
    { type_id: 1525, participant_id: 18710, score: { goals: 2, participant: 'away' }, description: 'CURRENT' },
    { type_id: 5, participant_id: 18551, score: { goals: 3, participant: 'home' }, description: 'PENALTY_SHOOTOUT' },
  ],
  events: [
    { id: 157120044, participant_id: 18710, type_id: 18, player_name: 'Álvaro Morata', related_player_name: 'Marco Asensio', minute: 63, extra_minute: null, sort_order: 5 },
    { id: 157120099, participant_id: 18551, type_id: 14, player_name: 'Scorer', related_player_name: null, minute: 70, extra_minute: 2, sort_order: 8 },
  ],
  statistics: [
    { id: 1, type_id: 42, participant_id: 18710, data: { value: 13 }, location: 'away' },
    { id: 2, type_id: 45, participant_id: 18551, data: { value: 23 }, location: 'home' },
  ],
  xgfixture: [],
};
...
test('toFixtureRow: xG(5304)が無ければ home/away_xg は null', () => {
  const row = toFixtureRow(fixtureDetail);
  assert.equal(row.home_xg, null);
  assert.equal(row.away_xg, null);
});

test('toFixtureRow: statistics に 5304 があれば xG を埋める', () => {
  const withXg = {
    ...fixtureDetail,
    statistics: [
      ...fixtureDetail.statistics,
      { id: 9, type_id: 5304, participant_id: 18551, data: { value: 1.73 }, location: 'home' },
      { id: 10, type_id: 5304, participant_id: 18710, data: { value: 0.92 }, location: 'away' },
    ],
  };
  const row = toFixtureRow(withXg);
  assert.equal(row.home_xg, 1.73);
  assert.equal(row.away_xg, 0.92);
});
