			...
			data: [
				{
					...
					player_id: 11,
					participant_id: 99,
					// display_name が名簿移行の正本。name が短縮形でも display_name を採る。
					player: { name: "A. Striker", display_name: "Alan Striker" },
...
test("toTopscorerRows は display_name 欠落時のみ name にフォールバック", () => {
	const rows = toTopscorerRows(
		{
			data: [
				{
	...
	assert.equal(rows[0].player_name, "Solo Name");
});
...
test("toXgStatRows: xgfixture を {fixture,team,type_id,value} 行へ畳む", () => {
	const detail = {
		...
		xgfixture: [
			...
			{
				participant_id: 18555,
				location: "home",
				type_id: 5305,
				data: { value: 0.4158 },
			},
			{ participant_id: null, location: "home", type_id: 7939, value: 1.65 },
		],
	};
