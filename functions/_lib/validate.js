...
export function validateConfig(input) {
	if (!isObj(input))
		return { ok: false, error: "config はオブジェクトである必要があります" };
	...
	for (const t of input.teams) {
		if (!isObj(t) || !isStr(t.code) || !CODE_RE.test(t.code)) {
			return {
				ok: false,
				error: `teams の code が不正です: ${JSON.stringify(t)}`,
			};
		}
		const code = t.code.toUpperCase();
		if (codes.has(code))
			return { ok: false, error: `teams の code が重複しています: ${code}` };
		if (!isStr(t.ja) || !t.ja.trim())
			return { ok: false, error: `teams の ja(名前) が必要です: ${code}` };
		codes.add(code);
		teams.push({
			code,
			ja: t.ja.trim(),
			flag: isStr(t.flag) ? t.flag : "",
			c: isStr(t.c) ? t.c : "#888888",
		});
	}
	...
	const champion =
		ri.champion == null || ri.champion === ""
			? null
			: String(ri.champion).toUpperCase();
	const runnerUp =
		ri.runnerUp == null || ri.runnerUp === ""
			? null
			: String(ri.runnerUp).toUpperCase();
	if (champion && !known(champion))
		return { ok: false, error: `result.champion が未登録: ${champion}` };
	if (runnerUp && !known(runnerUp))
		return { ok: false, error: `result.runnerUp が未登録: ${runnerUp}` };
	...
	for (const r of ["r16", "qf", "sf", "final"]) {
		...
		for (const c of arr) {
			if (!(isStr(c) && known(c.toUpperCase())))
				return { ok: false, error: `result.bracket.${r} に未登録コード: ${c}` };
		}
	...
	for (const r of ["r32", "r16", "qf", "sf"]) {
		...
		for (const c of arr) {
			if (!(isStr(c) && known(c.toUpperCase()))) {
				return {
					ok: false,
					error: `result.knockout.${r} に未登録コード: ${c}`,
				};
			}
		}
	...
	for (const k of Object.keys(tai)) {
		...
		if (!(isStr(v) && known(v.toUpperCase()))) {
			return {
				ok: false,
				error: `result.thirdAssign.${k} に未登録コード: ${v}`,
			};
		}
	...
	for (const g of tgi) {
		if (!isStr(g))
			return { ok: false, error: `result.thirdGroups に不正な値: ${g}` };
		const gu = g.toUpperCase();
		if (!/^[A-L]$/.test(gu))
			return { ok: false, error: `result.thirdGroups に不正なグループ: ${g}` };
		if (thirdGroups.includes(gu))
			return { ok: false, error: `result.thirdGroups にグループ重複: ${gu}` };
		thirdGroups.push(gu);
	}
	if (thirdGroups.length > 8)
		return { ok: false, error: "result.thirdGroups は最大8グループです" };
	...
	if (input.schedule != null) {
		if (!Array.isArray(input.schedule))
			return { ok: false, error: "schedule は配列が必要です" };
	...
	if (input.groups != null) {
		if (!isObj(input.groups))
			return { ok: false, error: "groups はオブジェクトが必要です" };
		const seenMembers = new Set();
		for (const k of Object.keys(input.groups)) {
			if (!GROUP_KEYS.includes(k))
				return { ok: false, error: `groups に不正なキー: ${k}` };
			const arr = input.groups[k];
			if (!Array.isArray(arr))
				return { ok: false, error: `groups.${k} は配列が必要です` };
			const norm = [];
			...
			for (const c of arr) {
				...
				if (!(isStr(c) && known(c.toUpperCase())))
					return { ok: false, error: `groups.${k} に未登録コード: ${c}` };
				const up = c.toUpperCase();
				if (localSeen.has(up))
					return { ok: false, error: `groups.${k} にチームの重複: ${up}` };
				if (seenMembers.has(up))
					return { ok: false, error: `チームが複数グループに所属: ${up}` };
				localSeen.add(up);
				seenMembers.add(up);
				norm.push(up);
			}
	...
	if (input.groupResult != null) {
		if (!isObj(input.groupResult))
			return { ok: false, error: "groupResult はオブジェクトが必要です" };
		for (const k of Object.keys(input.groupResult)) {
			if (!GROUP_KEYS.includes(k))
				return { ok: false, error: `groupResult に不正なキー: ${k}` };
			const arr = input.groupResult[k];
			if (!Array.isArray(arr))
				return { ok: false, error: `groupResult.${k} は配列が必要です` };
			...
			for (const c of arr) {
				...
				if (!up || !members.has(up))
					return { ok: false, error: `groupResult.${k} に所属外コード: ${c}` };
				if (localSeen.has(up))
					return { ok: false, error: `groupResult.${k} にチームの重複: ${up}` };
				localSeen.add(up);
				norm.push(up);
			}
			groupResult[k] = norm;
	...
	if (input.groupMatches != null) {
		if (!isObj(input.groupMatches))
			return { ok: false, error: "groupMatches はオブジェクトが必要です" };
		for (const k of Object.keys(input.groupMatches)) {
			if (!GROUP_KEYS.includes(k))
				return { ok: false, error: `groupMatches に不正なキー: ${k}` };
			const arr = input.groupMatches[k];
			if (!Array.isArray(arr))
				return { ok: false, error: `groupMatches.${k} は配列が必要です` };
			const norm = [];
			for (const m of arr) {
				if (!isObj(m))
					return { ok: false, error: `groupMatches.${k} の要素が不正です` };
				const a = isStr(m.a) ? m.a.toUpperCase() : "";
				const b = isStr(m.b) ? m.b.toUpperCase() : "";
				if (!known(a) || !known(b))
					return { ok: false, error: `groupMatches.${k} に未登録コード` };
				const sc = (v) => (v == null || v === "" ? null : v);
				const ga = sc(m.ga),
					gb = sc(m.gb);
				const okScore = (v) =>
					v === null || (Number.isInteger(v) && v >= 0 && v <= 99);
				if (!okScore(ga) || !okScore(gb))
					return { ok: false, error: `groupMatches.${k} のスコアが不正です` };
				norm.push({ a, b, ga, gb });
			}
			groupMatches[k] = norm;
		}
	...
	if (input.scorers != null) {
		if (!Array.isArray(input.scorers))
			return { ok: false, error: "scorers は配列が必要です" };
	...
	if (input.squads != null) {
		if (!isObj(input.squads))
			return { ok: false, error: "squads はオブジェクトが必要です" };
		for (const k of Object.keys(input.squads)) {
			const code = isStr(k) ? k.toUpperCase() : "";
			if (!known(code))
				return { ok: false, error: `squads に未登録コード: ${k}` };
			const arr = input.squads[k];
			if (!Array.isArray(arr))
				return { ok: false, error: `squads.${code} は配列が必要です` };
			const norm = [];
			for (const p of arr) {
				...
				if (pos && !POS_SET.has(pos)) {
					return {
						ok: false,
						error: `squads.${code} のポジションが不正です: ${p.pos}`,
					};
				}
	...
	if (input.aliases != null) {
		if (!Array.isArray(input.aliases))
			return { ok: false, error: "aliases は配列が必要です" };
		for (const a of input.aliases) {
			...
			if (!Array.isArray(a.variants)) {
				return {
					ok: false,
					error: `aliases.variants は配列が必要です: ${a.canonical}`,
				};
			}
			const variants = [];
			for (const v of a.variants) {
				if (!isStr(v) || !v.trim()) {
					return {
						ok: false,
						error: `aliases.variants に空文字: ${a.canonical}`,
					};
				}
			...
			if (a.smPlayerId != null) {
				if (!Number.isInteger(a.smPlayerId) || a.smPlayerId < 0) {
					return {
						ok: false,
						error: `aliases.smPlayerId が不正です: ${a.canonical}`,
					};
				}
