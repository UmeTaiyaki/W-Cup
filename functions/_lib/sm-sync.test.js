...
import {
  syncTypes, syncFixtureDetail, syncLive, syncSeasonFixtures, isInPlay, isFinished,
} from './sm-sync.js';
...
function fakeDb() {
  ...
  return {
    batched,
    prepare(sql) { return { bind: (...a) => ({ sql, a }) }; },
    batch: async (stmts) => { batched.push(stmts.length); return stmts.map(() => ({ success: true })); },
  };
}
...
test('syncTypes: pagination を has_more=false まで辿って upsert', async () => {
  const pages = {
    1: { data: [{ id: 1, name: 'A', code: 'a' }], pagination: { has_more: true } },
    2: { data: [{ id: 2, name: 'B', code: 'b' }], pagination: { has_more: false } },
  };
  let calls = 0;
  const client = { get: async (path, opts) => { calls++; return pages[opts.params.page]; } };
  const db = fakeDb();
...
test('syncTypes: maxPages で暴走を止める', async () => {
  const client = { get: async () => ({ data: [{ id: 1 }], pagination: { has_more: true } }) };
...
test('syncSeasonFixtures: data.fixtures を backfill(チャンク分割)', async () => {
  const mk = (id, h, a) => ({
    id, league_id: 732, season_id: 26618, starting_at: 'x', starting_at_timestamp: id, state_id: 1,
    participants: [
  ...
  const fixtures = Array.from({ length: 60 }, (_, i) => mk(i + 1, 100, 200 + i));
  const body = { data: { fixtures } };
  let captured = null;
  const client = { get: async (path, opts) => { captured = { path, opts }; return body; } };
  const db = fakeDb();
  ...
  assert.equal(db.batched.reduce((a, b) => a + b, 0), res.statements);
});
...
test('syncFixtureDetail: detail 取得→specs→batch', async () => {
  const detail = {
    data: {
      id: 7, league_id: 732, season_id: 26618, starting_at: '2026-06-11 19:00:00',
      starting_at_timestamp: 1, state_id: 5,
      participants: [
        { id: 10, name: 'A', meta: { location: 'home' } },
        { id: 20, name: 'B', meta: { location: 'away' } },
      ],
      scores: [
        { participant_id: 10, score: { goals: 2, participant: 'home' }, description: 'CURRENT' },
        { participant_id: 20, score: { goals: 1, participant: 'away' }, description: 'CURRENT' },
      ],
      events: [], statistics: [],
    },
  };
  let captured = null;
  const client = { get: async (path, opts) => { captured = { path, opts }; return detail; } };
  const db = fakeDb();
...
test('syncLive: livescores から各 fixture を upsert', async () => {
  const live = {
    data: [
      { id: 1, state_id: 3, participants: [{ id: 10, meta: { location: 'home' } }, { id: 20, meta: { location: 'away' } }], scores: [], events: [], statistics: [] },
      { id: 2, state_id: 3, participants: [{ id: 30, meta: { location: 'home' } }, { id: 40, meta: { location: 'away' } }], scores: [], events: [], statistics: [] },
    ],
  };
