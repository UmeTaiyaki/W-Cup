...
// xG の type_id（core/types で実在確認）。statistics に現れたら採用。
...
function currentGoals(detail, participant) {
  ...
  const hit = scores.find(
    (s) => s?.description === SCORE_CURRENT && s?.score?.participant === participant,
  );
...
function xgFor(detail, location) {
  ...
  const hit = stats.find((s) => s?.type_id === XG_TYPE_ID && s?.location === location);
  return hit?.data?.value ?? null;
}
...
export function toTypeRows(types) {
  ...
  return list
    .filter((t) => t?.id != null)
    .map((t) => ({ type_id: t.id, code: t.code ?? null, name: t.name ?? null }));
}
