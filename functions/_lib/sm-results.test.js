...
const koFixtures = [
	// Round of 32（FT）→ 勝者 ARG/MEX/FRA/ESP
	{
		...
		home: { app_code: "ARG", score: 3 },
		away: { app_code: "AUS", score: 0 },
	},
	{
		status: "FT",
		round_name: "Round of 32",
		home: { app_code: "MEX", score: 1 },
		away: { app_code: "POL", score: 0 },
	},
	{
		status: "FT",
		round_name: "Round of 32",
		home: { app_code: "FRA", score: 2 },
		away: { app_code: "SEN", score: 1 },
	},
	{
		status: "FT",
		round_name: "Round of 32",
...
test("deriveEliminated はFTのKO敗者を敗退集合に返す（未FTは除外）", () => {
	const ko = deriveEliminated(koFixtures);
	// R32敗者 AUS/POL/SEN/JPN、R16敗者 MEX/ESP、QF敗者 FRA。
	assert.deepEqual(
		ko.sort(),
		["AUS", "POL", "SEN", "JPN", "MEX", "ESP", "FRA"].sort(),
	);
});

test("deriveEliminated は決勝敗者(準優勝)も含む・未FT試合は無視", () => {
	const fx = [
		{
			status: "FT",
			round_name: "Final",
	...
	assert.deepEqual(deriveEliminated(fx), ["FRA"]);
});
