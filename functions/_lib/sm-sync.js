...
// fixture 詳細の include。xGFixture は実値(5304)を返さないと確認済みのため statistics に集約。
export const FIXTURE_DETAIL_INCLUDE = 'participants;scores;statistics;events';
...
export async function syncSeasonFixtures(footballClient, db, seasonId, now) {
  let body;
  try {
    body = await footballClient.get(`seasons/${seasonId}`, { include: SEASON_FIXTURES_INCLUDE });
  } catch (e) {
    console.error('syncSeasonFixtures: fetch failed', seasonId, e?.message);
    return { count: 0, error: e?.message };
  }
  const fixtures = Array.isArray(body?.data?.fixtures) ? body.data.fixtures : [];
...
export async function syncFixtureDetail(footballClient, db, fixtureId, now) {
  let body;
  try {
    body = await footballClient.get(`fixtures/${fixtureId}`, { include: FIXTURE_DETAIL_INCLUDE });
  } catch (e) {
...
export async function syncLive(footballClient, db, now) {
  let body;
  try {
    body = await footballClient.get('livescores/latest', { include: LIVE_INCLUDE });
  } catch (e) {
