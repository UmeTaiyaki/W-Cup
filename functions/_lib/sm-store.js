...
import { toTeamRows, toFixtureRow, toEventRows, toStatRows, toTypeRows } from './sm-ingest.js';
...
function fixtureStatement(row, updatedAt) {
  return {
    ...
    args: [
      row.sm_fixture_id, row.league_id, row.season_id, row.round_name, row.starting_at,
      row.starting_at_ts, row.state_id, row.state_short, row.home_team_id, row.away_team_id,
      row.home_score, row.away_score, row.home_xg, row.away_xg, row.venue, row.result_info,
      updatedAt,
    ],
  };
}
...
function eventStatement(row, updatedAt) {
  return {
    ...
    args: [
      row.sm_event_id, row.sm_fixture_id, row.minute, row.extra_minute, row.type, row.type_id,
      row.team_id, row.player_name, row.related_player_name, row.sort_order, updatedAt,
    ],
  };
}
...
// fixture 詳細1件 → teams/fixture/events/stats の upsert 文配列（純粋）
...
export function seasonFixturesStatements(fixtures, updatedAt) {
  ...
  const teamStmts = [...teamMap.values()].map((r) => teamStatement(r, updatedAt));
