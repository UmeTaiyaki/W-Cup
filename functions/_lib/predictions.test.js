...
test('emptyPred は全グループ・全枠・全ラウンドを持つ空予想を返す', () => {
  ...
  assert.deepEqual(p.thirdGroups, []);
  assert.deepEqual(Object.keys(p.knockout).sort(), ['qf','r16','r32','sf']);
});

test('validatePred はコードを大文字化し thirdGroups を正規化する', () => {
  const r = validatePred({
    champion: 'arg', runnerUp: '', topScorer: '  メッシ  ',
    groupRank: { A: ['mex', 'kor'] },
    thirdGroups: ['e', 'f', 'e', 'zz', 'g'], // 重複(e)・不正(zz)は除外
    knockout: { r16: ['esp'] },
  });
  ...
  assert.deepEqual(r.value.thirdGroups, ['E', 'F', 'G']);
  assert.deepEqual(r.value.knockout.r16, ['ESP']);
});

test('validatePred は thirdGroups を最大8グループにクランプする', () => {
  const r = validatePred({ thirdGroups: ['A','B','C','D','E','F','G','H','I','J'] });
  assert.equal(r.value.thirdGroups.length, 8);
});
