...
function GroupRankScreen({
	...
	const Card = ({ k }) => {
		...
		const auto4 =
			order.length === 3 ? members.find((c) => !order.includes(c)) : null;
		return (
			<div
				style={{
					background: T.card,
			...
			>
				...
				>
					...
					<span
						style={{
							fontSize: 11,
							color: order.length >= 3 ? T.accent : T.faint,
							fontWeight: 700,
						}}
					>
						{order.length >= 3 ? "完了" : `${order.length}/3`}
					</span>
				</div>
					...
					{members.map((code) => {
						...
						return (
							...
							>
								...
								{!picked && !isAuto && (
									<span style={{ fontSize: 12, color: T.faint }}>タップ</span>
								)}
							</button>
						);
					})}
	...
	return (
		<div style={{ padding: wide ? "4px 0 24px" : "4px 16px 16px" }}>
			...
			>
				...
				<span
					style={{
						...
						fontWeight: 800,
						fontSize: 15,
						color: doneCount === 12 ? T.accent : T.text,
					}}
				>
					{doneCount}
					<span style={{ color: T.faint, fontSize: 12 }}>/12組</span>
				</span>
			</div>
			<p
				style={{
			...
			>
				<DotBreak>
					各組をタップした順に1位→2位→3位。もう一度タップで取消。3位まで決めると4位は自動です。
				</DotBreak>
			</p>
			<div
				style={{
