...
function RankingScreen({ T, state, wide = false }) {
  ...
  const TableHead = () => (
    <div style={{ display: 'grid', gridTemplateColumns: COLS, gap: 10, alignItems: 'center',
      padding: '0 16px 10px', fontFamily: 'Archivo', fontWeight: 800, fontSize: 10,
      letterSpacing: 1.2, color: T.faint }}>
      <div>#</div><div>メンバー</div>
  ...
  const CatCell = ({ flag, text, ok, pts, color }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: ok ? T.text : T.faint, minWidth: 0,
        whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{text}</span>
      {pts > 0 && <span style={{ fontFamily: 'Archivo', fontWeight: 800, fontSize: 11,
        color, marginLeft: 'auto', flexShrink: 0 }}>+{pts}</span>}
    </div>
  );
  const RankTable = () => (
    <div style={{ background: T.card, borderRadius: 20, boxShadow: `inset 0 0 0 1px ${T.line}`,
      padding: '16px 0 6px', marginTop: 18 }}>
      <TableHead />
      <div>
        {scored.map((item, i) => {
