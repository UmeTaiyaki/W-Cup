...
// ===== ランキング画面 =======================================
// 得点の内訳（コア的中バッジ＋グループ順位/KO内訳）。ランキングのアコーディオンと
// 部屋のメンバー詳細で共有し、表記の二重管理を防ぐ。
...
function RankingScreen({ T, state, wide = false, onSelectMember = null }) {
	const M = state.members;
	...
	// onSelectMember 指定時はカードタップで詳細へ遷移（部屋のメンバー詳細）。
	// 未指定なら従来どおり行内アコーディオンで内訳を開閉する。
	const selectMember = onSelectMember
		? (item, i) =>
				onSelectMember({
					id: item.m.id,
					member: item.m,
			...
			<div>
				{scored.map((item, i) => {
					...
					return (
						<div
							key={item.m.id}
							onClick={selectMember ? () => selectMember(item, i) : undefined}
							style={{
								display: "grid",
								gridTemplateColumns: COLS,
								cursor: selectMember ? "pointer" : "default",
								gap: 10,
								alignItems: "center",
								padding: "11px 16px",
...
Object.assign(window, { CompareScreen, RankingScreen, MemberBreakdown });
