...
function scorerGroups(cfg) {
	...
	KEYS.forEach((k) =>
		((cfg.groups || {})[k] || []).filter(Boolean).forEach(push),
	);
...
function Login({ onOk, externalError }) {
	...
	if (stage === "otp") {
		return (
			<div style={{ maxWidth: 360, margin: "80px auto", padding: 24 }}>
				...
				<p style={{ fontSize: 13, color: "#9fb" }}>
					登録メールに送信した6桁のコードを入力してください。
				</p>
				<input
					type="text"
					inputMode="numeric"
	...
	return (
		<div style={{ maxWidth: 360, margin: "80px auto", padding: 24 }}>
			...
			/>
			{(err || externalError) && (
				<p style={{ color: "#FF6B6B", fontSize: 13 }}>{err || externalError}</p>
			)}
			<button
...
function Editor({ token, onAuthExpired, initial }) {
	...
	function setGroupMember(k, i, code) {
		setCfg((c) => {
			...
			const gr = {
				...c.groupResult,
				[k]: (c.groupResult[k] || []).filter((x) => !x || members.has(x)),
			};
	...
	function upResult(patch) {
		setCfg((c) => ({ ...c, result: { ...c.result, ...patch } }));
	}
	function upAliases(aliases) {
		setCfg((c) => ({ ...c, aliases }));
	}
	// 旧表記/別表記を名簿選手の canonical に紐付けてエイリアス登録（canonical は scoring.js と同一ロジック）。
	function registerAlias(variant, presetCode) {
		const wc = window.WC || {};
		if (!wc.canonicalKey || !wc.upsertAlias) {
			alert("エイリアス機能の読み込み中です。少し待って再度お試しください");
			return;
		}
		const name = window.prompt("割り当てる正式名（選手名簿の表記そのまま）");
		if (!name || !name.trim()) return;
		const code = (window.prompt("国コード（例: FRA）", presetCode || "") || "")
			.trim()
			.toUpperCase();
		if (!code) return;
		const canonical = wc.canonicalKey(`${name.trim()} (${code})`);
		upAliases(wc.upsertAlias(cfg.aliases || [], { canonical, variant }));
	}
	function upKnockout(round, arr) {
		setCfg((c) => ({
			...c,
			result: { ...c.result, knockout: { ...c.result.knockout, [round]: arr } },
		}));
	}
	...
	function fixturesForCfg(c, k) {
		...
		const gen = window.WC?.generateFixtures
			? window.WC.generateFixtures(members)
			: [];
		...
		return gen.map(({ a, b }) => {
			...
			return {
				a,
				b,
				ga: e ? (e.a === a ? e.ga : e.gb) : null,
				gb: e ? (e.a === a ? e.gb : e.ga) : null,
			};
		});
	}
	function fixturesFor(k) {
		return fixturesForCfg(cfg, k);
	}
	...
	function applyStandingsToRank(k) {
		setCfg((c) => {
			const members = (c.groups[k] || []).filter(Boolean);
			const rows = window.WC?.computeStandings
				? window.WC.computeStandings(members, fixturesForCfg(c, k))
				: [];
	...
	function toggleThirdGroup(g) {
		setCfg((c) => {
			...
			const alloc = window.WC?.resolveThirdAssign
				? window.WC.resolveThirdAssign(c.groupResult, next)
				: {};
			const ta = {};
			Object.keys(alloc).forEach((s) => {
				if (alloc[s]) ta[s] = alloc[s];
			});
			return {
				...c,
				result: { ...c.result, thirdGroups: next, thirdAssign: ta },
			};
		});
	}
	...
	// ---- 得点ランキング ----
	function addScorer() {
		setCfg((c) => ({ ...c, scorers: [...c.scorers, { name: "", goals: 0 }] }));
	}
	function setScorer(i, patch) {
		setCfg((c) => ({
			...c,
			scorers: c.scorers.map((s, j) => (j === i ? { ...s, ...patch } : s)),
		}));
	}
	function delScorer(i) {
		setCfg((c) => ({ ...c, scorers: c.scorers.filter((_, j) => j !== i) }));
	}
	...
	function setSquadPlayer(code, i, patch) {
		setCfg((c) => {
			const list = (c.squads?.[code] || []).map((p, j) =>
				j === i ? { ...p, ...patch } : p,
			);
	...
	function addSquadPlayer(code) {
		setCfg((c) => ({
			...
			squads: {
				...c.squads,
				[code]: [...(c.squads?.[code] || []), { name: "", pos: "", club: "" }],
			},
		}));
	}
	function delSquadPlayer(code, i) {
		setCfg((c) => ({
			...
			squads: {
				...c.squads,
				[code]: (c.squads?.[code] || []).filter((_, j) => j !== i),
			},
		}));
	}
	...
	function setSched(i, patch) {
		up({ schedule: sched.map((s, j) => (j === i ? { ...s, ...patch } : s)) });
	}
	function addSched() {
		up({
			schedule: [
				...sched,
				{ date: "", time: "", round: "", a: "", b: "", note: "" },
			],
		});
	}
	function delSched(i) {
		up({ schedule: sched.filter((_, j) => j !== i) });
	}
	...
	function resolveSchedTeams(s) {
		...
		const n = Number(m[1]);
		if (n === 104) {
			const f = der.winners?.sf || [];
			return [f[0] || null, f[1] || null];
		} // 決勝＝両準決勝の勝者
		if (n === 103) {
			// 3位決定戦＝両準決勝の敗者（勝者が確定している試合のみ）
			const sfm = der.matches?.sf || [];
			const sfw = der.winners?.sf || [];
			const loser = (i) =>
				sfw[i] ? (sfm[i] || []).find((t) => t && t !== sfw[i]) || null : null;
	...
	const teamName = (c) =>
		c
			? `${window.WC.TEAM?.[c]?.flag || ""}${window.WC.TEAM?.[c]?.ja || c}`
			: "未定";
		...
		<div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 18px 80px" }}>
			...
			<Section title="グループ（所属・試合スコア・最終順位）">
				{GROUP_KEYS.map((k) => {
					...
					const memberTeams = teams.filter((t) =>
						(cfg.groups[k] || []).includes(t.code),
					);
					const ranks = cfg.groupResult[k] || [];
					const nameOf = (code) => {
						const t = teams.find((x) => x.code === code);
						return t ? `${t.flag} ${t.ja}` : code;
					};
					return (
						<div
							key={k}
						...
						>
							...
							>
								{[0, 1, 2, 3].map((i) => (
									<TeamSelect
										...
										teams={teams}
										value={members[i]}
										onChange={(c) => setGroupMember(k, i, c)}
									/>
								))}
							</div>
							<div style={{ fontSize: 12, color: "#9aa", margin: "6px 0 4px" }}>
								試合スコア
							</div>
							{fixturesFor(k).map((m, idx) => (
								...
								>
									...
									<input
										...
										value={m.ga ?? ""}
										onChange={(e) =>
											setMatchScore(k, idx, "ga", e.target.value)
										}
									...
									<input
										...
										value={m.gb ?? ""}
										onChange={(e) =>
											setMatchScore(k, idx, "gb", e.target.value)
										}
										style={{ ...inputStyle, width: 48, textAlign: "center" }}
									/>
									<span
							...
							>
								...
								<button
									onClick={() => applyStandingsToRank(k)}
									style={{
										...inputStyle,
										cursor: "pointer",
										fontSize: 12,
			...
			<Section title="正解（勝敗）">
				...
				>
					<label>
						...
						<TeamSelect
							teams={teams}
							value={cfg.result.champion}
							onChange={(c) => upResult({ champion: c })}
						/>
					</label>
					<label>
						準優勝{" "}
						<TeamSelect
							teams={teams}
							value={cfg.result.runnerUp}
							onChange={(c) => upResult({ runnerUp: c })}
						/>
					</label>
				</div>
				<p style={{ fontSize: 12, color: "#9aa", margin: "4px 0 10px" }}>
					ノックアウトはベスト32から。各ラウンドの勝者を選択（採点はこの集合との一致で加点）。
				</p>
							...
							<div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
								{teams.map((t) => {
									...
									return (
										<button
											key={t.code}
											onClick={() => toggleKnockout(r.key, t.code)}
											style={{
												...inputStyle,
												cursor: "pointer",
												background: on ? "#B6FF3C" : "#0f1a15",
												color: on ? "#0A1410" : "#ccc",
				...
				>
					3位通過した8組（ノックアウト表の対戦カード用）
				</div>
				<p style={{ fontSize: 12, color: "#6a7", margin: "0 0 8px" }}>
					実際に3位で勝ち上がった8組を選択。ベスト32のどの枠に入るかは FIFA
					公式の組み合わせ表（Annex C）で自動決定されます（
					{(cfg.result.thirdGroups || []).length}/8組）。
				</p>
				<div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
				...
				{(cfg.result.thirdGroups || []).length === 8 &&
					window.WC?.resolveThirdAssign &&
					(() => {
						const alloc = window.WC.resolveThirdAssign(
							cfg.groupResult,
							cfg.result.thirdGroups,
						);
						const struct = (window.WC.BRACKET_STRUCTURE || {}).r32 || [];
						const winnerSeed = {};
						struct.forEach((m) => {
							if (Array.isArray(m.bottom?.wc)) winnerSeed[m.id] = m.top;
						});
						return (
							<div style={{ marginTop: 10 }}>
								<div style={{ fontSize: 11, color: "#6a7", marginBottom: 4 }}>
									自動割当プレビュー
								</div>
								<div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
			...
			<Section title="得点王">
				...
				<p style={{ fontSize: 12, color: "#9aa", margin: "0 0 8px" }}>
					選手名簿の全選手から選択（⭐は注目候補）。予想がこの選手と一致すると加点されます。
				</p>
				<select
					className="wc-scorer-select"
					value={cfg.result.topScorer || ""}
					onChange={(e) => upResult({ topScorer: e.target.value })}
					style={{ ...inputStyle, maxWidth: 320 }}
				>
					<option value="">未選択</option>
					{cfg.result.topScorer &&
						!knownScorerValues.has(cfg.result.topScorer) && (
							<option value={cfg.result.topScorer}>
								{cfg.result.topScorer}（旧データ）
							</option>
						)}
					{scorerFavs.length > 0 && (
					...
					{scorerGrps.map((g) => (
						<optgroup key={g.code} label={g.label}>
							{g.players.map((p, i) => (
								<option key={g.code + i} value={`${p.name} (${g.code})`}>
									{"　" + scorerText(p)}
								</option>
							))}
						</optgroup>
				...
				{cfg.result.topScorer &&
					!knownScorerValues.has(cfg.result.topScorer) && (
						<div style={{ marginTop: 8 }}>
							<button
								onClick={() => registerAlias(cfg.result.topScorer)}
				...
				<p style={{ fontSize: 12, color: "#9aa", margin: "0 0 8px" }}>
					選手名簿の全選手から選択。予想アプリのグループ画面に表示される、実際の得点者一覧。
				</p>
				{cfg.scorers.map((s, i) => (
					<div
						key={i}
					...
					>
						...
						>
							...
							{scorerFavs.length > 0 && (
								<optgroup label="⭐ 得点王候補">
									{scorerFavs.map((f, j) => (
										<option key={"fav" + j} value={f.value}>
											{"　" + f.label}
										</option>
									))}
								</optgroup>
							...
							{scorerGrps.map((g) => (
								<optgroup key={g.code} label={g.label}>
									{g.players.map((p, j) => (
										<option key={g.code + j} value={`${p.name} (${g.code})`}>
											{"　" + scorerText(p)}
										</option>
									))}
								</optgroup>
						...
						<input
							...
							onChange={(e) =>
								setScorer(i, {
									goals: Math.max(0, parseInt(e.target.value, 10) || 0),
								})
							}
						...
						<button
							onClick={() => delScorer(i)}
							style={{ ...inputStyle, cursor: "pointer", color: "#FF6B6B" }}
						>
							削除
						</button>
					</div>
			...
			<Section title="得点王エイリアス">
				<p style={{ fontSize: 12, color: "#9aa", margin: "0 0 8px" }}>
					表記ゆれ（旧予想・SportMonks名）を正規キーに畳んで採点一致させます。予想/正解データは変更しません。同じ選手の別表記をまとめて登録してください。
				</p>
				{(cfg.aliases || []).map((a, i) => (
					<div
						key={i}
					...
					>
						...
						<div style={{ fontSize: 12, color: "#9cf", margin: "4px 0" }}>
							...
							{a.smPlayerId != null && (
								<span style={{ color: "#7a7" }}>　smId:{a.smPlayerId}</span>
							)}
						</div>
						<button
							onClick={() =>
								upAliases((cfg.aliases || []).filter((_, j) => j !== i))
							}
							style={{ ...inputStyle, cursor: "pointer", fontSize: 12 }}
						>
							削除
				...
				<button
					onClick={() => {
						...
						if (!wc.canonicalKey || !wc.upsertAlias) {
							alert(
								"エイリアス機能の読み込み中です。少し待って再度お試しください",
							);
							return;
						}
						const name = window.prompt("正式名（選手名簿の表記そのまま）");
						if (!name || !name.trim()) return;
						...
						const variant = window.prompt(
							"変種（旧表記/別表記。例: VINI JR. (BRA)）",
						);
						const canonical = wc.canonicalKey(`${name.trim()} (${code})`);
						upAliases(
							wc.upsertAlias(cfg.aliases || [], {
								canonical,
						...
						>
							...
							>
								...
								<span style={{ fontWeight: 700, flex: 1 }}>
									{t.ja}{" "}
									<span style={{ color: "#6a7", fontWeight: 400 }}>
										({t.code})
									</span>
								</span>
									...
									{list.map((p, i) => (
										...
										>
											...
											<select
												value={p.pos || ""}
												onChange={(e) =>
													setSquadPlayer(t.code, i, { pos: e.target.value })
												}
											...
											<input
												...
												placeholder="選手名"
												onChange={(e) =>
													setSquadPlayer(t.code, i, { name: e.target.value })
												}
												style={{
													...inputStyle,
													flex: "1 1 120px",
											...
											<input
												...
												placeholder="所属クラブ"
												onChange={(e) =>
													setSquadPlayer(t.code, i, { club: e.target.value })
												}
												style={{
													...inputStyle,
													flex: "2 1 180px",
											...
											/>
											<button
												onClick={() => delSquadPlayer(t.code, i)}
												style={{
													...inputStyle,
													cursor: "pointer",
													color: "#FF6B6B",
			...
			<Section title="試合日程（参考）">
				<p style={{ fontSize: 12, color: "#9aa", margin: "0 0 12px" }}>
					...
					実際の対戦国を自動表示します（緑字）。スロット表記（1A＝A組1位
					等）はそのまま保持されます。
				</p>
				{sched.map((s, i) => {
						...
						<div key={i} style={{ marginBottom: 6 }}>
							...
							>
								<input
									type="date"
									value={s.date}
									onChange={(e) => setSched(i, { date: e.target.value })}
									style={inputStyle}
								/>
								<input
									type="time"
								...
								<input
									value={s.round}
									placeholder="GL/R16..."
									onChange={(e) => setSched(i, { round: e.target.value })}
									style={{ ...inputStyle, width: 90 }}
								/>
								<input
									value={s.a}
									placeholder="A"
									onChange={(e) => setSched(i, { a: e.target.value })}
								...
								<input
									value={s.b}
									placeholder="B"
									onChange={(e) => setSched(i, { b: e.target.value })}
									style={{ ...inputStyle, width: 80 }}
								/>
								<input
									value={s.note}
									placeholder="メモ"
									onChange={(e) => setSched(i, { note: e.target.value })}
									style={{ ...inputStyle, flex: 1 }}
								/>
								<button
									onClick={() => delSched(i)}
...
function Admin() {
	...
	function logout() {
		setToken("");
		setCfg(null);
		setLoadError("セッションの有効期限が切れました。再度ログインしてください");
	}
	...
	async function afterLogin(tok) {
		...
		try {
			...
			const rawKo =
				baseResult.knockout && typeof baseResult.knockout === "object"
					? baseResult.knockout
					: {};
			const knockout = {
				r32: rawKo.r32 || [],
			...
			const thirdAssign =
				baseResult.thirdAssign && typeof baseResult.thirdAssign === "object"
					? baseResult.thirdAssign
					: {};
			const thirdGroups = Array.isArray(baseResult.thirdGroups)
				? baseResult.thirdGroups
				: [];
			const cfg = {
				...raw,
				result: {
					champion: baseResult.champion ?? null,
					runnerUp: baseResult.runnerUp ?? null,
					topScorer: baseResult.topScorer ?? "",
					knockout,
					thirdAssign,
					thirdGroups,
				},
				...
				groupResult:
					raw.groupResult && typeof raw.groupResult === "object"
						? raw.groupResult
						: {},
				groupMatches:
					raw.groupMatches && typeof raw.groupMatches === "object"
						? raw.groupMatches
						: {},
			...
			setLoadError("設定の取得に失敗しました。もう一度お試しください");
		}
		setLoading(false);
	}
