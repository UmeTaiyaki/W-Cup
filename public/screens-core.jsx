...
function EditBtn({ T, onClick, label = "編集" }) {
	return (
		...
		>
			<Icon name="edit" size={14} color={T.accent} sw={2} />
			{label}
		</button>
	);
}
		...
		>
			...
			>
				{code ? (
					...
					>
						<Icon name={icon} size={20} color={T.faint} />
					</div>
				)}
			</div>
			<div style={{ marginTop: 8 }}>
				<FitText
					text={code ? window.WC.TEAM[code]?.ja : scorer || "未選択"}
					max={17}
					min={11}
					weight={800}
					color={T.text}
...
function SummaryScreen({
				...
				{M.map((m) => {
					...
					return (
						...
						>
							...
							{c ? (
								<Flag code={p.champion} size={22} />
							) : (
								<span style={{ fontSize: 22 }}>🏳️</span>
							)}
							<span style={{ fontWeight: 700, color: T.sub, fontSize: 14 }}>
								{c ? c.ja : "—"}
							</span>
						</div>
					);
	...
	const ChampDist = () => {
		const counts = {};
		M.forEach((m) => {
			const v = state.preds[m.id].champion;
...
function InputScreen({
	...
	const viewState = state || {
		current: member.id,
		members: [member],
		preds: { [member.id]: pred },
	};

	return (
		<div
			style={{
				padding: wide ? "4px 0 24px" : "4px 16px 16px",
				display: "flex",
				flexDirection: "column",
				gap: 14,
			}}
					...
					{!confirm ? (
						<button
							...
							style={{
								...
								background: "transparent",
								color: canRemove ? T.danger : T.faint,
