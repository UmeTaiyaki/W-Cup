...
function KnockoutScreen({
	...
	const der = window.WC.deriveKnockoutAuto(
		pred.groupRank || {},
		ta,
		pred.knockout || {},
		forced,
	...
	React.useEffect(() => {
		...
		const d = window.WC.deriveKnockoutAuto(
			pred.groupRank || {},
			ta,
			pred.knockout || {},
			forced,
		);
		if (JSON.stringify(d.winners) !== JSON.stringify(pred.knockout || {}))
			setKnockout(d.winners);
	}, [
		pred.champion,
		pred.runnerUp,
		JSON.stringify(pred.groupRank),
		JSON.stringify(pred.thirdGroups),
		JSON.stringify(pred.knockout),
	]);
	...
	const TeamBtn = ({ team, isWinner, dimmed, onClick, half, placeholder }) => (
		<button
			...
			style={{
				...
				padding: "0 9px",
				fontFamily: "inherit",
				borderRadius: isWinner ? 9 : 0,
				opacity: dimmed ? 0.4 : 1,
				minWidth: 0,
			}}
		>
			{team ? (
				<Flag code={team} size={17} style={{ flexShrink: 0 }} />
			) : (
				<span style={{ fontSize: 17, flexShrink: 0 }}>⚪️</span>
			)}
			<span
				style={{
					fontSize: team ? 12.5 : 11.5,
	...
	const Header = () => (
		<div style={{ padding: wide ? "4px 0 12px" : "4px 16px 12px" }}>
			...
			>
				<DotBreak>
					対戦カードはグループ順位予想と3位割当から自動で決まります。各試合で勝者をタップ。決勝の勝敗は「優勝予想」（コア）で決まります。
				</DotBreak>
			</p>
		</div>
	);
						...
						{ROUNDS.map((r, i) => (
							<div
								...
								style={{
									...
									height: 7,
									borderRadius: 4,
									background: i === ri ? T.accent : T.line,
									cursor: "pointer",
									transition: ".2s ease",
								}}
							/>
							...
							return (
								...
								>
									<TeamBtn
										...
										dimmed={w && w !== teams[0]}
										onClick={() => pick(round, idx, teams[0])}
										half={40}
										placeholder={seeds[0]}
									/>
									<div
										style={{ height: 1, background: T.line, margin: "0 8px" }}
									/>
									<TeamBtn
										team={teams[1]}
										isWinner={w && w === teams[1]}
										dimmed={w && w !== teams[1]}
										onClick={() => pick(round, idx, teams[1])}
										half={40}
										placeholder={seeds[1]}
									/>
								</div>
	...
	const fitScale = availWidth
		? Math.max(0.5, Math.min(1.2, (availWidth - 8) / contentW))
		: 1;
	...
	return (
		<div>
						...
						>
							...
							>
								...
								<div style={{ fontSize: 22 }}>
									{champ ? <Flag code={champ.code} size={22} /> : "🏆"}
								</div>
								<div
									style={{
										fontWeight: 800,
			...
			<OptionSaveBar
				...
				style={{ maxWidth: 520, margin: "18px auto 0" }}
				hint="「保存」を押すと予想を保存して予想ハブに戻ります。押すまで保存されません。"
			/>
		</div>
	);
