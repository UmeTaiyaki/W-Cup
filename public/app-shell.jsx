		...
		return (
			...
			>
				<Icon
					...
					sw={active ? 2.2 : 1.9}
				/>
				<span style={{ fontWeight: active ? 800 : 700, fontSize: 15 }}>
					{tb.label}
				</span>
			</button>
		);
							...
							return (
								<button
									...
									style={{
										...
										boxShadow: active ? `inset 0 0 0 1px ${m.c}59` : "none",
										transition: ".15s ease",
									}}
								>
...
function RightRail({ T, state, member, pred, goTab }) {
	...
	const scored = state.members
		.map((m) => ({ m, s: window.WC.scoreMember(state.preds[m.id]) }))
	...
	const PickLine = ({ icon, color, label, code, scorer }) => (
		<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
			...
			<span
				style={{ fontSize: 13, color: T.faint, width: 48, fontWeight: 700 }}
			>
				{label}
			</span>
			{code ? (
				<Flag code={code} size={18} style={{ verticalAlign: "middle" }} />
			) : (
				<span style={{ fontSize: 18 }}>{scorer ? "⚽️" : "—"}</span>
			)}
			<span
				style={{
					fontWeight: 700,
		...
		>
			...
			>
				...
				>
					...
					>
						全部見る
						<Icon name="chevron" size={13} color={T.accent} />
					</button>
				</div>
	...
	return (
		<div
			...
			style={{
				...
				alignItems: "center",
				gap: 7,
				padding: tappable ? "11px 17px" : "8px 13px",
				borderRadius: 999,
				background: conf.bg,
				color: conf.fg,
				fontFamily: "inherit",
