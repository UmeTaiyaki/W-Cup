...
> **Task1スパイク確定事項**: ノックアウト構造は `stage.name`（"Round of 32"/"Round of 16"/"Quarter-finals"/"Semi-finals"/"3rd Place Final"/"Final"/"Group Stage"）に入る。`round.name` は群リーグの節番号("1"/"2"/"3")または KO で null。よって **stage.name を優先**して round_name に採用する。

```javascript
import { toFixtureRow } from "./sm-ingest.js";

test("toFixtureRow は stage.name を round_name に採用（KO構造は stage 由来）", () => {
  const row = toFixtureRow({ id: 1, participants: [], scores: [], stage: { id: 9, name: "Round of 16" } });
  assert.equal(row.round_name, "Round of 16");
});

test("toFixtureRow は stage を round より優先", () => {
  const row = toFixtureRow({ id: 2, participants: [], scores: [], stage: { name: "Final" }, round: { name: "3" } });
  assert.equal(row.round_name, "Final");
});

test("toFixtureRow は stage が無ければ round.name、どちらも無ければ null", () => {
  assert.equal(toFixtureRow({ id: 3, participants: [], scores: [], round: { name: "1" } }).round_name, "1");
  assert.equal(toFixtureRow({ id: 4, participants: [], scores: [] }).round_name, null);
});
```
...
```
（注: Task1スパイク時点で topscorers は `count:0`（本大会前で空）。`GOAL_TYPE_ID=208` は SportMonks 一般値の仮置きで、本大会でデータが入り次第 `?action=topscorers` の実レスポンスで type_id を検証・修正する。空の間は得点王 `topScorer=""` でグレースフルに無表示。`app_code` は participant.short_code を採点が使う3文字コードに変換できないため、ここでは null とし、配信時に sm_teams JOIN で解決する。）
...
```
（注: 入力文字列は `round_name`（=Task2 で stage.name を格納）。Task1スパイク実値 "Round of 32"/"Round of 16"/"Quarter-finals"/"Semi-finals"/"3rd Place Final"/"Final"/"Group Stage" と一致。表記ゆれ対策で小文字化＋記号除去して突合する。）
